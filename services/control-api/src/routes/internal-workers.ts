  ...
  router.get(INTERNAL_WORKERS_PATH, (_request, response) => {
    response.json({
      workers: options.workerRegistry.listWorkers().map((worker) => ({
        ...
        stabilityGateStatus: worker.stabilityGateStatus ?? "unstable",
        stabilityPassCount: worker.stabilityPassCount ?? 0,
        stabilityTargetPasses: worker.stabilityTargetPasses ?? 2,
        lastValidationAt: worker.lastValidationAt ?? null,
        lastValidationResult: worker.lastValidationResult ?? null,
