...
function createTestRuntime(
  ...
  const root = mkdtempSync(
    join(tmpdir(), "control-api-post-phase21-disconnected-runtime-followup-")
  );
  tempDirectories.push(root);

  const postPhase21DisconnectedRuntimeFollowupStatePath = join(
    root,
    "infra",
    "data",
  ...
  if (latestState) {
    mkdirSync(dirname(postPhase21DisconnectedRuntimeFollowupStatePath), {
      recursive: true
    });
    writeFileSync(
...
describe("internal post-phase21 disconnected runtime follow-up route", () => {
  it("returns latest null when no post-phase21 follow-up state exists", async () => {
  ...
  it("returns the parsed latest post-phase21 follow-up payload when it exists", async () => {
    const latestState = {
  ...
  it("parses a UTF-8 BOM-prefixed post-phase21 follow-up artifact without failing", async () => {
    const root = mkdtempSync(
      join(tmpdir(), "control-api-post-phase21-disconnected-runtime-followup-bom-")
    );
    tempDirectories.push(root);
