...
function createTestRuntime(
  ...
  const root = mkdtempSync(
    join(tmpdir(), "control-api-post-parity-disconnected-runtime-remediation-")
  );
  tempDirectories.push(root);

  const postParityDisconnectedRuntimeRemediationStatePath = join(
    root,
    "infra",
    "data",
  ...
  if (latestState) {
    mkdirSync(dirname(postParityDisconnectedRuntimeRemediationStatePath), {
      recursive: true
    });
    writeFileSync(
...
describe("internal post-parity disconnected runtime remediation route", () => {
  it("returns latest null when no post-parity remediation state exists", async () => {
    const { app, runtime } = createTestRuntime();
    cleanupCallbacks.push(() => runtime.dispose());

    const response = await request(app)
      .get("/internal/post-parity-disconnected-runtime-remediation/latest")
  ...
  it("returns the parsed latest post-parity remediation payload when it exists", async () => {
    const latestState = {
  ...
  it("parses a UTF-8 BOM-prefixed post-parity remediation artifact without failing", async () => {
    const root = mkdtempSync(
      join(tmpdir(), "control-api-post-parity-disconnected-runtime-remediation-bom-")
    );
    tempDirectories.push(root);
