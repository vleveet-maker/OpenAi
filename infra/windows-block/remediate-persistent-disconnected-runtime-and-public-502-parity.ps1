...
$phaseDir = Join-Path $repoRoot ".planning\phases\20-deployed-windows-runtime-parity-backport-and-persistent-disconnected-runtime-public-502-remediation"
$upstreamRemediationScriptPath = Join-Path $PSScriptRoot "remediate-persistent-disconnected-runtime-and-public-502.ps1"
$temporaryRemediationJsonPath = Join-Path $phaseDir "20-TEMP-UPSTREAM-REMEDIATION.json"
$temporaryRemediationMarkdownPath = Join-Path $phaseDir "20-TEMP-UPSTREAM-REMEDIATION.md"

if (-not (Test-Path $upstreamRemediationScriptPath)) {
  throw "Missing Phase 20 remediation dependency: $upstreamRemediationScriptPath"
}

if ([string]::IsNullOrWhiteSpace($LatestJsonPath)) {
  $LatestJsonPath = Join-Path $repoRoot "infra\data\runtime-parity-backport-remediation\latest.json"
}

if ([string]::IsNullOrWhiteSpace($OutputJsonPath)) {
  $OutputJsonPath = Join-Path $phaseDir "20-REMEDIATION-SUMMARY.json"
}

if ([string]::IsNullOrWhiteSpace($OutputMarkdownPath)) {
  $OutputMarkdownPath = Join-Path $phaseDir "20-REMEDIATION-SUMMARY.md"
}
...
if (-not (Test-Path $temporaryRemediationJsonPath)) {
  throw "Phase 20 remediation could not find upstream remediation output at $temporaryRemediationJsonPath"
}

$remediationResult = ConvertFrom-JsonSafe -Raw (Get-Content -Path $temporaryRemediationJsonPath -Raw)

if ($null -eq $remediationResult) {
  throw "Phase 20 remediation could not parse upstream remediation output."
}
...
$beforeStageSnapshot = Get-UpstreamStageSnapshot -RemediationResult $remediationResult -StageName "before_followup"
$stageSnapshots = [ordered]@{
  before_parity_remediation = $beforeStageSnapshot
  after_runtime_parity_assertion = $beforeStageSnapshot
...
$preRemediationReadyCount =
  if ($null -ne $remediationResult.preFollowupReadyCount) {
    [int]$remediationResult.preFollowupReadyCount
  } else {
    0
  }
$preRemediationTotalWorkers =
  if ($null -ne $remediationResult.preFollowupTotalWorkers) {
    [int]$remediationResult.preFollowupTotalWorkers
  } else {
    0
  }
$postRemediationReadyCount =
  if ($null -ne $remediationResult.postFollowupReadyCount) {
    [int]$remediationResult.postFollowupReadyCount
  } else {
    0
  }
$postRemediationTotalWorkers =
  if ($null -ne $remediationResult.postFollowupTotalWorkers) {
    [int]$remediationResult.postFollowupTotalWorkers
  } else {
    0
  }
...
$verdict =
  if ([string]$remediationResult.verdict -eq "followup_ready_for_smoke") {
    "parity_backport_ready_for_smoke"
  } else {
    "hold_rollout"
  }
$summary =
  if ($verdict -eq "parity_backport_ready_for_smoke") {
    "Runtime parity backport remediation stayed green end-to-end, so the host is ready for the mandatory smoke rerun."
  } else {
...
$result = [pscustomobject][ordered]@{
  ...
  parityBackportFiles = @($parityBackportFiles)
...
$lines = @()
$lines += "# Runtime Parity Backport Remediation Summary"
$lines += ""
...
$lines += "- parityBackportFiles: $(@($result.parityBackportFiles) -join ', ')"
