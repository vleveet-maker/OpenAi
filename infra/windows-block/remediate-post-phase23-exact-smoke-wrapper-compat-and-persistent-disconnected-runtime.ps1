...
$phaseDir = Join-Path $repoRoot ".planning\phases\24-deployed-windows-post-phase23-exact-smoke-wrapper-compat-backport-and-persistent-disconnected-runtime-forced-host-public-owner-502-remediation"
$upstreamRemediationScriptPath = Join-Path $PSScriptRoot "remediate-post-phase22-smoke-wrapper-parity-and-persistent-disconnected-runtime.ps1"
$temporaryRemediationJsonPath = Join-Path $phaseDir "24-TEMP-UPSTREAM-REMEDIATION.json"
$temporaryRemediationMarkdownPath = Join-Path $phaseDir "24-TEMP-UPSTREAM-REMEDIATION.md"

if (-not (Test-Path $upstreamRemediationScriptPath)) {
  throw "Missing Phase 24 remediation dependency: $upstreamRemediationScriptPath"
}

if ([string]::IsNullOrWhiteSpace($LatestJsonPath)) {
  $LatestJsonPath = Join-Path $repoRoot "infra\data\post-phase23-exact-smoke-wrapper-compat-remediation\latest.json"
}

if ([string]::IsNullOrWhiteSpace($OutputJsonPath)) {
  $OutputJsonPath = Join-Path $phaseDir "24-COMPAT-REMEDIATION-SUMMARY.json"
}

if ([string]::IsNullOrWhiteSpace($OutputMarkdownPath)) {
  $OutputMarkdownPath = Join-Path $phaseDir "24-COMPAT-REMEDIATION-SUMMARY.md"
}

$compatCheckFiles = @(
  $smokeWrapperCompatFiles | ForEach-Object {
    Join-Path $repoRoot $_
  }
)
$missingCompatFiles = @(
  $compatCheckFiles | Where-Object {
    -not (Test-FileContainsLiteral -Path $_ -Literal $scriptCompatibilityVersion)
  }
)
$smokeWrapperCompatStatus =
  if ($missingCompatFiles.Count -eq 0) {
    "archive_chain_ready"
  } else {
    "marker_missing"
  }
...
if (-not (Test-Path $temporaryRemediationJsonPath)) {
  throw "Phase 24 remediation could not find upstream remediation output at $temporaryRemediationJsonPath"
}

$remediationResult = ConvertFrom-JsonSafe -Raw (Get-Content -Path $temporaryRemediationJsonPath -Raw)

if ($null -eq $remediationResult) {
  throw "Phase 24 remediation could not parse upstream remediation output."
}
...
$beforeStageSnapshot = Get-UpstreamStageSnapshot -RemediationResult $remediationResult -StageName "before_phase23_parity_recovery"
$compatCheckSnapshot = [pscustomobject][ordered]@{
  checkedAt = (Get-Date).ToString("o")
  smokeWrapperCompatStatus = $smokeWrapperCompatStatus
  compatibilityMarker = $scriptCompatibilityVersion
  checkedFiles = @($smokeWrapperCompatFiles)
  missingCompatFiles = @($missingCompatFiles)
}
$stageSnapshots = [ordered]@{
  before_phase24_exact_compat_recovery = $beforeStageSnapshot
  after_exact_smoke_wrapper_compat_check = $compatCheckSnapshot
...
$verdict =
  if (
    [string]$remediationResult.verdict -eq "parity_recovered_ready_for_smoke" -and
    $smokeWrapperCompatStatus -eq "archive_chain_ready"
  ) {
...
$summary =
  if ($verdict -eq "compat_backported_ready_for_smoke") {
    "Exact smoke-wrapper compat markers survived the archive chain and the post-Phase-23 remediation remained green enough for the mandatory smoke rerun."
  } else {
    "Hold rollout: smokeWrapperCompatStatus=$smokeWrapperCompatStatus, preRemediationReadyCount=$preRemediationReadyCount/$preRemediationTotalWorkers, postRemediationReadyCount=$postRemediationReadyCount/$postRemediationTotalWorkers, dominantRuntimeBlocker=$dominantRuntimeBlocker, firstFailingHop=$firstFailingHop."
  }
...
$result = [pscustomobject][ordered]@{
  ...
  smokeWrapperCompatStatus = $smokeWrapperCompatStatus
  smokeWrapperCompatFiles = @($smokeWrapperCompatFiles)
  missingCompatFiles = @($missingCompatFiles)
...
$lines = @()
$lines += "# Post-Phase-23 Exact Smoke-Wrapper Compat Remediation Summary"
$lines += ""
...
$lines += ""
$lines += "## Exact Smoke-Wrapper Compat"
$lines += ""
$lines += "- smokeWrapperCompatStatus: $($result.smokeWrapperCompatStatus)"
$lines += "- smokeWrapperCompatFiles: $(@($result.smokeWrapperCompatFiles) -join ', ')"
$lines += "- missingCompatFiles: $(if (@($result.missingCompatFiles).Count -gt 0) { @($result.missingCompatFiles) -join ', ' } else { 'none' })"
$lines += ""
$lines += "## Post-Phase-23 Remediation"
$lines += ""
