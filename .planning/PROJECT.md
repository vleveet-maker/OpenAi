...
- Phase 37 is now complete with `hold_rollout`: Ubuntu SSH, nginx canonical upstream, bearer-token resolution, external `/healthz`, authenticated `/v1/models`, and reverse tunnel listeners `14021..14027/14040` are green, but external `/v1/chat/completions` now stops at the honest browser-account gate `bootstrap_auth_required`; the next step is manual account confirmation/sign-in, not more proxy or Ubuntu work
...
- Phase 36 is now complete with `hold_rollout`: the token/SSH/listener wrapper, latest route, admin section, focused tests, full tests, and build are green; direct SSH retry to `77.66.186.75:2222` resolved the bearer token and authenticated `/v1/models=200`, but Ubuntu listeners `14021..14027` plus `14040` are missing, so authenticated external chat was correctly skipped.
...
| Restore Ubuntu reverse-tunnel listeners before more browser or proxy debugging | Phase 36 direct SSH retry proved token and public models are green, but Ubuntu has no listeners on `14021..14027` or `14040`; without those reverse tunnels, public chat cannot reach the Windows browser workers | - Recorded during Phase 36 completion |
