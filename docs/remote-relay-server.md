...
## Live Public Owner Truth

Current preserve-first truth on `2026-03-31`:

- outside-client probes to `77.66.186.75` now deliberately land on Ubuntu-side `nginx`
- that Ubuntu owner now proxies the public API contract to the preserved Windows `Caddy` edge on `192.168.88.250`
- the Windows browser block stays preserved behind that owner and no longer needs to pretend it directly owns the public IP

This matters because the old public-edge ownership blocker is now resolved.
...
The deliberate front door is now a two-layer preserve-first edge:

1. Ubuntu `nginx` owns public `77.66.186.75`
2. Ubuntu proxies `/healthz`, `/api/relay/*`, and `/v1/*` to the preserved Windows edge on `192.168.88.250`
...
Local Ubuntu relay truth remains available separately on:

- `http://127.0.0.1:8080/healthz`
- `http://127.0.0.1:4010` behind that local-only path

The preserved Windows public API edge stays behind Ubuntu and keeps raw Windows internals private.
...
The repeatable rollout-confidence command for the current live topology is:
...
- internal pool readiness
- current operator failure counts
- public `healthz`
- public `v1/models`
- one canary public chat completion on `shared-6`

It does not claim that:

- the full public worker set is ready for automatic widening
- HTTPS trust hardening is complete
- public ingress ownership needs to be revisited
